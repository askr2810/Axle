...
Object.assign(SIMS, {
  ...
  pid: { t: ["PID-regulator", "PID controller"], p: [["Kp", "K_p", 0, 20, 0.5, 2, ""], ["Ki", "K_i", 0, 10, 0.25, 0, ""], ["Kd", "K_d", 0, 5, 0.1, 0, ""]],
    ...
    f: v => { const r = smPidRun(v.Kp, v.Ki, v.Kd), m = smStepInfo(r.ys, r.dt, 1, v.Ki > 0 ? 1 : v.Kp / (1 + v.Kp)), Y = smSeries(r.ys, r.dt), U = smSeries(r.us, r.dt);
      ...
      return { m, out: [[T("oversving", "overshoot"), smPct(m.os)], [T("innsvingning", "settling"), smSec(m.ts)], [T("avvik", "error"), smN(m.ess * 100, 1) + " %"]], svg: g.svg }; } },
  second: { t: ["Andreordens stegrespons", "Second-order step response"], p: [["z", "ζ", 0.05, 2, 0.05, 0.2, ""], ["wn", "ω_n", 1, 10, 0.5, 3, "rad/s"]],
    ...
    f: v => { const dt = 0.01, ys = smSecond(v.z, 6 * v.wn, dt), m = smStepInfo(ys, dt / v.wn, 1, 1), Y = t => smSeries(ys, dt)(t * v.wn);
      ...
      return { m, out: [[T("oversving", "overshoot"), smPct(m.os)], [T("innsvingning", "settling"), smSec(m.ts)], [T("poler", "poles"), v.z < 1 ? `−${smN(v.z * v.wn)} ± ${smN(v.wn * Math.sqrt(1 - v.z * v.z))}j` : T("reelle", "real")]], svg: g.svg }; } },
  rlc: { t: ["RLC-krets: stegrespons", "RLC circuit: step response"], p: [["R", "R", 5, 200, 5, 20, "Ω"], ["L", "L", 10, 200, 10, 100, "mH"], ["C", "C", 10, 200, 10, 100, "µF"]],
    ...
    f: v => { const L = v.L / 1000, C = v.C / 1e6, w0 = 1 / Math.sqrt(L * C), z = v.R / 2 * Math.sqrt(C / L), dt = 0.02, tm = 30 / w0 * 1000;
      ...
      return { m, out: [["ζ = (R/2)√(C/L)", smN(z, 3)], ["f₀ = 1/(2π√(LC))", smN(m.f0, 1) + " Hz"], [T("oversving", "overshoot"), smPct(m.os)]], svg: g.svg }; } },
  // ---------- signaler ----------
  fourier: { t: ["Fourierrekke", "Fourier series"], p: [["w", ["bølge (0 firkant, 1 sag, 2 trekant)", "wave (0 square, 1 saw, 2 triangle)"], 0, 2, 1, 0, ""], ["N", ["antall ledd", "number of terms"], 1, 25, 1, 3, ""]],
    ...
    f: v => { const w = Math.round(v.w), P = Math.PI;
      ...
      return { m, out: [[T("bølge", "wave"), [T("firkant", "square"), T("sagtann", "sawtooth"), T("trekant", "triangle")][w]], [T("RMS-feil", "RMS error"), smPct(m.err)]], svg: g.svg }; } },
  alias: { t: ["Sampling og aliasing", "Sampling and aliasing"], p: [["f", ["signal f", "signal f"], 1, 20, 1, 3, "Hz", 1], ["fs", ["samplingsfrekvens f_s", "sample rate f_s"], 2, 40, 1, 20, "Hz", 3]],
    ...
    f: v => { const k = Math.round(v.f / v.fs), fa = Math.abs(v.f - k * v.fs), fs2 = v.f - k * v.fs, ns = Math.floor(v.fs);
      ...
      return { m: { fa }, out: [["Nyquist f_s/2", smN(v.fs / 2, 1) + " Hz"], [T("ser ut som", "looks like"), smN(fa, 0) + " Hz"], ["OK?", v.fs > 2 * v.f ? T("ja", "yes") : T("aliasing!", "aliasing!")]], svg: g.svg }; } },
  // ---------- matematikk ----------
  quad: { t: ["Andregradsfunksjonen", "The quadratic function"], p: [["a", "a", -3, 3, 0.5, 1, ""], ["b", "b", -6, 6, 0.5, 0, ""], ["c", "c", -6, 6, 0.5, -2, ""]],
    ...
    f: v => { const D = v.b * v.b - 4 * v.a * v.c, xv = v.a ? -v.b / (2 * v.a) : NaN, yv = v.a ? v.c - v.b * v.b / (4 * v.a) : NaN;
      ...
      return { m: { D, xv, yv }, out: [["D = b² − 4ac", smN(D)], [T("nullpunkter", "zeros"), roots.length ? roots.map(r => smN(r)).join(T(" og ", " and ")) : T("ingen", "none")], [T("topp/bunn", "vertex"), v.a ? `(${smN(xv)}, ${smN(yv)})` : "–"]], svg: g.svg }; } },
  trig: { t: ["Treff kurven: A·sin(bx + c) + d", "Match the curve: A·sin(bx + c) + d"], p: [["A", "A", 0.5, 3, 0.5, 1, ""], ["b", "b", 0.5, 3, 0.5, 1, ""], ["c", "c", -180, 180, 15, 0, "°"], ["d", "d", -2, 2, 0.5, 0, ""]],
    ...
    f: v => { const tg = [[2, 1, 0, 1], [1, 2, 0, -1], [1.5, 1, 90, 0], [0.5, 3, -45, 1.5]], gi = Math.min(tg.length - 1, Math.max(0, v._g | 0)), [A, b, c, d] = tg[gi], R = Math.PI / 180;
      ...
      return { m: { hit: dev < 0.02, dev }, out: [[T("periode 2π/b", "period 2π/b"), smN(2 / v.b, 2) + "π"], [T("avstand til målet", "distance to target"), smN(dev)]], svg: g.svg.replace('class="fg-red" d', 'class="fg-red" stroke-dasharray="6 4" d') }; } },
  taylor: { t: ["Taylorpolynom", "Taylor polynomial"], p: [["fn", ["funksjon (0 sin, 1 cos, 2 eˣ)", "function (0 sin, 1 cos, 2 eˣ)"], 0, 2, 1, 0, ""], ["n", ["grad n", "degree n"], 0, 15, 1, 1, ""]],
    ...
    f: v => { const w = Math.round(v.fn), der = k => w === 0 ? [0, 1, 0, -1][k % 4] : w === 1 ? [1, 0, -1, 0][k % 4] : 1, F = [Math.sin, Math.cos, Math.exp][w];
      ...
      return { m, out: [[T("feil i x = 3", "error at x = 3"), m.err < 1e-4 ? "< 0,0001".replace(",", T(",", ".")) : smN(m.err, 4)], [T("antall ledd ≠ 0", "non-zero terms"), String(Array.from({ length: v.n + 1 }, (_, k) => der(k)).filter(Boolean).length)]], svg: g.svg }; } },
  ...
  euler: { t: ["Eulers metode: steglengde og stabilitet", "Euler's method: step size and stability"], p: [["h", ["steglengde h", "step size h"], 0.02, 1.2, 0.02, 0.4, ""], ["meth", ["metode (0 Euler, 1 Heun)", "method (0 Euler, 1 Heun)"], 0, 1, 1, 0, ""]],
    ...
    f: v => { const n = Math.ceil(4 / v.h - 1e-9), f = y => -2 * y, pts = [[0, 1]]; let y = 1, err = 0;
      ...
      return { m, out: [[T("antall steg", "steps"), String(n)], [T("maksfeil", "max error"), m.err > 99 ? "> 99" : smN(m.err, 4)], ["|1 − 2h|", smN(Math.abs(1 - 2 * v.h), 2)]], svg: g.svg }; } },
  newton: { t: ["Newtons metode", "Newton's method"], p: [["x0", "x₀", -3, 3, 0.1, 3, "", 1], ["n", ["iterasjoner", "iterations"], 0, 6, 1, 1, ""]],
    ...
    f: v => { const f = x => x * x - 2, d = x => 2 * x; let x = v.x0, broken = false; const seg = [];
      ...
      return { m, out: [["xₙ", broken ? T("f′(x) = 0 – tangenten treffer aldri", "f′(x) = 0 – the tangent never hits") : smN(x, 7)], [T("feil", "error"), broken ? "–" : err < 1e-7 ? "< 1e−7" : smN(err, 7)]], svg: g.svg }; } },
  gd: { t: ["Gradientnedstigning", "Gradient descent"], p: [["eta", ["læringsrate η", "learning rate η"], 0.05, 1.1, 0.05, 0.1, ""], ["n", ["steg", "steps"], 1, 30, 1, 5, ""]],
    ...
    f: v => { let w = -2; const path = [w]; for(let i = 0; i < v.n; i++){ w = w - v.eta * 2 * (w - 3); path.push(w); } const m = { w, dist: Math.abs(w - 3) };
      ...
      return { m, out: [["w", Math.abs(w) > 1e5 ? "±∞" : smN(w, 4)], [T("avstand til minimum", "distance to minimum"), m.dist > 1e5 ? "∞" : smN(m.dist, 4)], [T("faktor 1 − 2η", "factor 1 − 2η"), smN(1 - 2 * v.eta)]], svg: g.svg }; } },
  // ---------- fasthet ----------
  mohr: { t: ["Mohrs sirkel", "Mohr's circle"], p: [["sx", "σ_x", -100, 100, 10, 60, "MPa", 1], ["sy", "σ_y", -100, 100, 10, -20, "MPa", 3], ["tau", "τ_xy", -60, 60, 5, 30, "MPa", 2]],
    ...
    f: v => { const C = (v.sx + v.sy) / 2, R = Math.hypot((v.sx - v.sy) / 2, v.tau), s1 = C + R, s2 = C - R, th = 0.5 * Math.atan2(2 * v.tau, v.sx - v.sy) * 180 / Math.PI, k = 0.62, X = s => 160 + s * k, Y = t => 90 + t * k;
      ...
      return { m: { R, s1, s2 }, out: [["σ₁", smN(s1, 1) + " MPa"], ["σ₂", smN(s2, 1) + " MPa"], [T("τ_maks", "τ_max"), smN(R, 1) + " MPa"], ["θ_p", smN(th, 1) + "°"]], svg: `
  ...
  buckle: { t: ["Knekking av søyle (Euler)", "Column buckling (Euler)"], p: [["d", ["diameter d", "diameter d"], 20, 80, 5, 40, "mm", 3], ["L", "L", 1, 6, 0.5, 3, "m", 2], ["k", ["opplagring (0–3)", "supports (0–3)"], 0, 3, 1, 1, ""]],
    ...
    f: v => { const K = [2, 1, 0.7, 0.5][Math.round(v.k)], nm = [T("fast–fri", "fixed–free"), T("ledd–ledd", "pinned–pinned"), T("fast–ledd", "fixed–pinned"), T("fast–fast", "fixed–fixed")][Math.round(v.k)];
      ...
      return { m: { P, Pthin: v.d > 20 ? Pcr(v.d - 5) : 0 }, out: [["P_cr = π²EI/(KL)²", smN(P, 1) + " kN"], ["K", smN(K, 1) + " (" + nm + ")"]], svg: `
  ...
  // ---------- termo og økonomi ----------
  carnot: { t: ["Carnot: virkningsgrad og COP", "Carnot: efficiency and COP"], p: [["Th", "T_H", 300, 900, 10, 600, "K"], ["Tc", "T_C", 250, 400, 5, 300, "K"]],
    ...
    f: v => { const ok = v.Th > v.Tc, eta = ok ? 1 - v.Tc / v.Th : 0, cop = ok ? v.Th / (v.Th - v.Tc) : Infinity, copK = ok ? v.Tc / (v.Th - v.Tc) : Infinity, wq = smClamp(eta * 60, 2, 60);
      return { m: { eta, cop, copK }, out: [["η = 1 − T_C/T_H", ok ? smPct(eta * 100) : "–"], ["COP_VP = T_H/(T_H − T_C)", ok ? smN(cop) : "–"], ["COP_K = T_C/(T_H − T_C)", ok ? smN(copK) : "–"]], svg: `
  ...
  npv: { t: ["Nåverdi og internrente", "Net present value and IRR"], p: [["a", ["innbetaling per år", "cash flow per year"], 50, 400, 10, 150, T("tusen kr", "k NOK")], ["n", ["levetid", "lifetime"], 3, 20, 1, 10, T("år", "yr")], ["r", ["kalkulasjonsrente", "discount rate"], 0, 20, 0.5, 5, "%"]],
